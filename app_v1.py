...
st.set_page_config(page_title="A's ERP 2.1.1 終極版", layout="wide")
...
if check_password():
    ...
    with st.sidebar:
        ...
        if st.button("🔄 同步 Shopify 最新數據"):
            st.cache_data.clear()
            st.toast("✅ 數據已從 Shopify 重新獲取")
            st.rerun()
        st.divider()
        if st.button("🚪 安全登出"):
            st.session_state["password_correct"] = False
            st.rerun()
        st.info("版本: V 2.1.1 (穩定修復版)")
    ...
    # --- 3. 容錯處理：自動偵測庫存欄位名稱 ---
    # 解決 KeyError 的核心邏輯：動態匹配「現貨庫存」或「現有庫存」
    ...
    def export_dashboard_pdf(df_p, df_o, sales_stats, fig_obj):
        ...
        p.drawString(50, height - 50, "營運分析與物流報表 (V2.1.1)")
        
        # 指標卡
        metrics = [
            ("總銷售額", f"${df_o['Total_USD'].sum():,.2f}"), 
            ("總利潤", f"${sum(sales_stats.get(x['產品名稱'], 0) * x['毛利'] for x in products):,.2f}"), 
            ("訂單數", str(len(df_o)))
        ]
        ...
        img_buf.seek(0)
        p.drawImage(ImageReader(img_buf), 50, 80, width=500, height=220)
        p.showPage(); p.save()
    ...
    with tab1:
        ...
        st.write("### 🛒 產品即時清單")
        # 使用安全列選取，防止 KeyError
        safe_cols = ["產品名稱", "售價", "成本"]
        if stock_col: safe_cols.insert(1, stock_col)
        
        st.dataframe(df_p[safe_cols], use_container_width=True)
        ...
        if stock_col:
            ...
            if not low_stock.empty:
                st.warning(f"目前有 {len(low_stock)} 項產品庫存低於臨界值")
    ...
    with tab2:
        ...
        # 核心：必須先定義 fig 才能被 PDF 和 pyplot 同時調用
    ...
    with tab3:
        st.header("🔍 廣告文案合規檢查")
        ad_text = st.text_area("請輸入待檢查的文案：", height=200, placeholder="例如：本產品能迅速治癒...")
        if st.button("開始掃描"):
            if ad_text:
                forbidden = ["治癒", "療效", "速效", "神藥", "根治", "副作用"]
                found = [w for w in forbidden if w in ad_text]
                if found:
                    st.error(f"❌ 發現禁語：{', '.join(found)}")
                else:
